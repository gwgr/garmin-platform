...
- set `FRONTEND_BIND_IP` in `/opt/garmin-platform/.env` to the VPS Tailscale IPv4 address so Docker publishes the frontend only on Tailscale instead of all interfaces
- example: `FRONTEND_BIND_IP=100.80.139.125`
- verify with `gp-ps` or `docker compose ... ps` that the frontend shows the Tailscale bind, for example `100.80.139.125:3000->3000/tcp`, and not `0.0.0.0:3000->3000/tcp`
...
- that installer adds documented helpers for `gp-help`, `gp-env`, `gp-app`, `gp-gitpull`, `gp-deploy`, `gp-backup`, `gp-sync-once`, `gp-sync-status`, `gp-reprocess`, `gp-ps`, `gp-logs`, `gp-logs-backend`, `gp-logs-frontend`, `gp-logs-postgres`, `gp-backend-health`, and `gp-timer-status`
...
- the repo-owned CI setup actions have now been refreshed to newer maintained majors (`actions/setup-python@v6`, `actions/setup-node@v6`, `astral-sh/setup-uv@v7`), and the follow-up CI run completed cleanly
